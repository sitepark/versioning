...
abstract class MapBasedElementBranchSet<E extends SortedElementSet>
		implements ElementBranchSet {
	...
	public boolean retainAll(final Collection<?> objects) {
		for (final Map.Entry<Branch, E> entry
				: this.branchMap.entrySet()) {
	...
	public Iterator<SpecificationElement> iterator() {
		return new Iterator<SpecificationElement>() {
			private final Iterator<E> iterator
					= MapBasedElementBranchSet.this.branchMap
				.values()
				.iterator();
			...
			public SpecificationElement next() {
				if (!this.current.hasNext()) {
					...
					this.next = this.iterator.hasNext()
						? this.iterator.next().iterator()
						: null;
				}
	...
	protected SortedElementBranchSet getIntersection(
			final MapBasedElementBranchSet<?> other) {
		...
		for (final Branch branch : branches) {
			intersection = this.branchMap.get(branch)
				.getIntersection(other.branchMap.get(branch));
		...
		return new UnmodifiableSortedElementBranchSet(
				this.branchMap.entrySet()
					...
					.collect(Collectors.toMap(
							Map.Entry::getKey,
							e -> e.getValue().unmodifiableClone())));
	}
	...
	public boolean equals(final Object other) {
		...
		return this.branchMap.equals(
				((MapBasedElementBranchSet<?>)other).branchMap);
	}
}