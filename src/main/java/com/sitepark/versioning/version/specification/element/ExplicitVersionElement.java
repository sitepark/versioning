 ...
 */
public final class ExplicitVersionElement
		extends SpecificationElement.VersionBased {
	...
	public ComparisonResult compareToBoundariesBased(
		...
		final int cmpLower = other.getBoundaries()
			.getLower()
			.compareTo(this.version);
		if (cmpLower > 0) {
			return ComparisonResult.LOWER;
		}
		final int cmpUpper = other.getBoundaries()
			.getUpper()
			.compareTo(this.version);
		if (cmpUpper < 0) {
			return ComparisonResult.HIGHER;
