...
public final class Boundaries<
			L extends Boundary & Boundary.Lower,
			U extends Boundary & Boundary.Upper>
		...
		if (!(this.lower instanceof WithVersion)) {
			if (!(this.upper instanceof WithVersion)) {
				throw new InvalidBoundariesException(
					"atleast one Boundary has to be limited");
			}
			...
			if (this.upper instanceof WithVersion) {
				if (this.lower.compareTo(this.upper) >= 0) {
					throw new InvalidBoundariesException(
							"the lower Boundary has to be smaller"
								+ " than the upper Boundary");
				}
				if (!Objects.equals(
						((WithVersion)this.upper).version.getBranch(),
						this.branch)) {
					throw new InvalidBoundariesException(
							"boundaries cannot have different branches");
				}
			}
		}
	}
	 ...
	 */
	public <
				A extends Boundary & Boundary.Lower,
				B extends Boundary & Boundary.Upper>
			Optional<Boundaries<?, ?>> getIntersection(
					final Boundaries<A, B> other) {
		...
		try {
			return Optional.of(
					new Boundaries<>(
							lowerCmp < 0 ? other.lower : this.lower,
							upperCmp > 0 ? other.upper : this.upper));
