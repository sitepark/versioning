...
class VersionsSpecificationParseExecutor {
	 ...
	 *                    not valid
	 */
	public VersionsSpecification execute()
	...
	private void addVersionRange()
			throws ParseException, InvalidBoundariesException {
		this.builder.addVersionRange(
				new VersionRangeElement(this.buildBoundaries()));
	}
	...
	private Boundaries<?, ?> buildBoundaries()
			throws ParseException, InvalidBoundariesException {
		if (this.currentLowerVersionLength == 0) {
			if (this.currentUpperVersionLength == 0) {
				throw new InvalidBoundariesException(
						"VersionRanges have to have "
							+ "atleast one limiting Boundary");
			}
