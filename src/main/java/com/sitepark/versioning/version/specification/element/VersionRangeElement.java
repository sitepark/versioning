 ...
 * @see Boundaries
 */
public final class VersionRangeElement
		extends SpecificationElement.BoundariesBased {
	...
	public Optional<SpecificationElement> getIntersectionWithBoundariesBased(
		...
		return this.boundaries.getIntersection(other.getBoundaries())
			.map(VersionRangeElement::new);
	}

	@Override
	public ComparisonResult compareToVersionBased(
			final SpecificationElement.VersionBased other) {
		final int cmpLower = this.boundaries.getLower()
			.compareTo(other.getVersion());
		if (cmpLower > 0) {
			return ComparisonResult.HIGHER;
		}
		final int cmpUpper = this.boundaries.getUpper()
			.compareTo(other.getVersion());
		if (cmpUpper < 0) {
	...
	public ComparisonResult compareToBoundariesBased(
			final SpecificationElement.BoundariesBased other) {
		int cmp = this.boundaries.getLower()
			.compareTo(other.getBoundaries().getLower());
		if (cmp > 0) {
			if (this.boundaries.getLower().compareTo(
					other.getBoundaries().getUpper()) > 0) {
				return ComparisonResult.HIGHER;
			}
			if (this.boundaries.getUpper().compareTo(
					other.getBoundaries().getUpper()) > 0) {
		...
		if (cmp < 0) {
			if (this.boundaries.getUpper().compareTo(
					other.getBoundaries().getLower()) < 0) {
				return ComparisonResult.LOWER;
			}
			if (this.boundaries.getUpper().compareTo(
					other.getBoundaries().getUpper()) < 0) {
		...
		cmp = this.boundaries.getUpper()
			.compareTo(other.getBoundaries().getUpper());
		if (cmp > 0) {
			return ComparisonResult.INTERSECTS_COMPLETELY;
		}
		if (cmp < 0) {
	...
	public boolean equals(final Object other) {
		...
		return this.boundaries.equals(
				((VersionRangeElement)other).boundaries);
	}
}
