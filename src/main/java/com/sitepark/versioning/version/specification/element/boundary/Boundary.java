...
public abstract class Boundary implements Comparable<Boundary>, Serializable {
	 ...
	 */
	static abstract class WithVersion extends Boundary {
	...
	private interface Comparison<A extends Boundary, B extends Boundary> {
		 ...
		 * greater.
		 */
		Comparison<? super WithVersion, ? super WithVersion>
				COMPARE_VERSIONS_OR_LARGER = (a, b) -> {
		 ...
		 */
		Comparison<? super WithVersion, ? super WithVersion>
				COMPARE_VERSIONS_OR_SMALLER = (a, b) -> {
					final int cmp = a.getVersion().compareTo(b.getVersion());
					return cmp != 0 ? cmp : -1;
				};
	...
	private enum BoundaryType {
		...
		<E extends Boundary> BoundaryType(
				final Class<E> clazz,
				final BoundaryComparator<E> comparator) {
			this.clazz = clazz;
		 ...
		 */
		static <E extends Boundary> BoundaryType getByClass(
				final Class<E> clazz) {
	...
	private static final class BoundaryComparator<E extends Boundary> {
		private final Comparison<? super E, ? super UnlimitedUpperBoundary>
				unlimitedUpper;
		private final Comparison<? super E, ? super ExclusiveUpperBoundary>
				exclusiveUpper;
		private final Comparison<? super E, ? super InclusiveUpperBoundary>
				inclusiveUpper;
		private final Comparison<? super E, ? super UnlimitedLowerBoundary>
				unlimitedLower;
		private final Comparison<? super E, ? super ExclusiveLowerBoundary>
				exclusiveLower;
		private final Comparison<? super E, ? super InclusiveLowerBoundary>
				inclusiveLower;

		BoundaryComparator(
				final Comparison<? super E, ? super UnlimitedUpperBoundary>
						unlimitedUpper,
				final Comparison<? super E, ? super ExclusiveUpperBoundary>
						exclusiveUpper,
				final Comparison<? super E, ? super InclusiveUpperBoundary>
						inclusiveUpper,
				final Comparison<? super E, ? super UnlimitedLowerBoundary>
						unlimitedLower,
				final Comparison<? super E, ? super ExclusiveLowerBoundary>
						exclusiveLower,
				final Comparison<? super E, ? super InclusiveLowerBoundary>
						inclusiveLower) {
		...
		int compareBoundaries(
				final Boundary left,
				final Boundary right) {
			switch (BoundaryType.getByClass(right.getClass())) {
				case UNLIMITED_UPPER_BOUNDARY:
					return this.unlimitedUpper.compare(
							(E)left,
							(UnlimitedUpperBoundary)right);
				case EXCLUSIVE_UPPER_BOUNDARY:
					return this.exclusiveUpper.compare(
							(E)left,
							(ExclusiveUpperBoundary)right);
				case INCLUSIVE_UPPER_BOUNDARY:
					return this.inclusiveUpper.compare(
							(E)left,
							(InclusiveUpperBoundary)right);
				case UNLIMITED_LOWER_BOUNDARY:
					return this.unlimitedLower.compare(
							(E)left,
							(UnlimitedLowerBoundary)right);
				case EXCLUSIVE_LOWER_BOUNDARY:
					return this.exclusiveLower.compare(
							(E)left,
							(ExclusiveLowerBoundary)right);
				case INCLUSIVE_LOWER_BOUNDARY:
					return this.inclusiveLower.compare(
							(E)left,
							(InclusiveLowerBoundary)right);
				default:
