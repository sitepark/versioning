...
public class VersionFormatter {
	...
	public static final VersionFormatter DEFAULT_BASE_VERSION_FORMATTER
			= new VersionFormatter(
					VersionFormatter.DEFAULT_FORMAT + ":-SNAPSHOT:");
	/**
	...
	public static final VersionFormatter DEFAULT_CONCRETE_VERSION_FORMATTER
			= new VersionFormatter(
					VersionFormatter.DEFAULT_FORMAT
	...
	private List<FormatElement> parseFormat(final String format)
		...
		for (int i = 0; i < format.length(); i++) {
			...
			if (startIndex > openingColonIndex) {
				...
				startIndex = i + 1;
				result.add(i - 1 == openingColonIndex
					? new StringElement(":")
					: KeywordElement.parse(
							format.substring(openingColonIndex + 1, i)));
			}
		}
		if (startIndex <= openingColonIndex) {
			throw new MissingColonFormatException(
					format.substring(openingColonIndex));
		}
	 ...
	 */
	private static abstract class FormatElement {
	...
	private static final class KeywordElement extends FormatElement {
		 ...
		 */
		static KeywordElement parse(final String string)
	...
	private enum Keyword {
		...
		TIMESTAMP(e -> e instanceof ConcreteSnapshotVersion
			? ((ConcreteSnapshotVersion)e).getTimestamp()
			: null),
		BUILDNUMBER(e -> e instanceof ConcreteSnapshotVersion
			? ((ConcreteSnapshotVersion)e).getBuildnumber()
			: null),
		QUALIFIERS(e -> !e.getQualifiers().isEmpty()
			? String.join("-", e.getQualifiers())
			: null);

		private transient final Function<Version, ? extends Object> converter;
	 ...
	 */
	public static class MissingColonFormatException
