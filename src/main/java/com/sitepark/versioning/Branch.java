...
public class Branch implements Comparable<Branch>, Serializable {
	...
	public Branch(final String value) {
		if (value.indexOf(' ') != -1) {
			throw new IllegalArgumentException(
					"Branches cannot contain spaces");
		}
		if (value.indexOf('-') != -1) {
			throw new IllegalArgumentException(
					"Branches cannot contain hyphens");
		}
		this.value = value.equalsIgnoreCase(Branch.DEVELOP_VALUE)
				|| value.length() == 0
	...
	private Object readResolve() throws ObjectStreamException {
		if (this.value.equalsIgnoreCase(Branch.DEVELOP_VALUE)
				|| value.length() == 0) {
